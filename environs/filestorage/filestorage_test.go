...
func (s *filestorageSuite) TestURL(c *gc.C) {
	...
	c.Assert(err, gc.IsNil)
	c.Assert(url, gc.Equals, "file://"+utils.StripDriveLetter(expectedpath))
}
...
func (s *filestorageSuite) TestPathRelativeToHome(c *gc.C) {
	...
	url, err := reader.URL("")
	c.Assert(err, gc.IsNil)
	c.Assert(url, gc.Equals, "file://"+utils.StripDriveLetter(filepath.Join(homeDir, dirName)))
}
...
func (s *filestorageSuite) TestRelativePath(c *gc.C) {
	...
	url, err := reader.URL("")
	c.Assert(err, gc.IsNil)
	c.Assert(url, gc.Equals, "file://"+utils.StripDriveLetter(dir)+"/a")
}
