...
func (s *ValidateSuite) assertMatch(c *gc.C, stream string) {
	...
	params := &simplestreams.MetadataLookupParams{
		...
		Sources: []simplestreams.DataSource{
			simplestreams.NewURLDataSource("test", "file://"+utils.StripDriveLetter(metadataPath), utils.VerifySSLHostnames)},
	}
	...
	c.Check(resolveInfo, gc.DeepEquals, &simplestreams.ResolveInfo{
		Source:    "test",
		Signed:    false,
		IndexURL:  "file://" + utils.StripDriveLetter(path.Join(metadataPath, "streams/v1/index.json")),
		MirrorURL: "",
	})
}
