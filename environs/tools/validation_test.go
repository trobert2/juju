...
func (s *ValidateSuite) toolsURL() string {
	return "file://" + utils.StripDriveLetter(path.Join(s.metadataDir, "tools"))
}
...
func (s *ValidateSuite) TestExactVersionMatch(c *gc.C) {
	...
	c.Check(resolveInfo, gc.DeepEquals, &simplestreams.ResolveInfo{
		Source:    "test",
		Signed:    false,
		IndexURL:  "file://" + utils.StripDriveLetter(path.Join(s.metadataDir, "tools/streams/v1/index.json")),
		MirrorURL: "",
	})
}
